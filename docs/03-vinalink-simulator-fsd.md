...
```mermaid
graph LR
    User((Doanh chủ / Cố vấn)) -->|Input: N, r, split, Rank, F1, dup| P1[Module Tiếp nhận Tham số]
    P1 -->|Config Data| P2[Engine Tính toán Doanh số & Tăng trưởng S-curve]
    P2 -->|Monthly CV Data| P3[Engine Thăng cấp & Kiểm tra F1 Gánh]
    P3 -->|Title & Rank State| P4[Engine Tính toán 7 Nguồn thu]
    P4 -->|Income Breakdown| P5[Module Hiển thị & Trích xuất Báo cáo]
...
Hệ thống cung cấp giao diện tương tác hỗ trợ người dùng thiết lập 8 tham số chính:

| Tên Tham số | Mã Tham số | Loại / Đơn vị | Khoảng Giá trị | Mặc định | Mô tả Chức năng |
| :--- | :--- | :--- | :--- | :--- | :--- |
| Quy mô hệ thống tối đa | `n_total` | Integer / Người | $100 - 50.000$ | $10.000$ | Tổng quy mô thành viên tích lũy sau 36 tháng. |
...
| Số F1 nòng cốt | `n_f1` | Integer / Người | $1 - 10$ | $3$ | Số lượng F1 trực hệ Doanh chủ bảo trợ trực tiếp. |
...
2. **Hoa hồng Bảo trợ trực tiếp ($Income_2$):** Sơ đồ trực hệ $10\%$ F1-F2, $5\%$ F3-F5. Gold bảo trợ Gold nhận $2.000\text{ CV}$ ($2.000.000\text{ VNĐ}$).
...
5. **Hoa hồng Cộng hưởng - Matching Bonus ($Income_5$):** Hưởng $10\%$ F1-F2 và $5\%$ F3-F5 trên GVC tuyến dưới. Số NPP ở tầng $k$: $N_{npp}(k) = n\_f1 \times dup\_factor^{k-1}$.
...
Tăng trưởng quy mô hệ thống tích lũy tuân theo hàm Logistic S-curve:
